function [path_reuseable, sample_interval, speed_data_pre, ...
    ...
    road_width = 7.5 * 3; % 3.5m * 3 lanes
    heading_max = 35.0 / 57.3;
    ...
    dddl_bound.lb = ...
            ...
            ego_para.wheelbase / reference_speed * sec(control_constraint.steer_max)^2 - ...
...
function [obstacle_avoid_bound, obstacle_bound, obs_info] = GenerateObstacles( ...
   ...
   obs_info.width = ego_para.width + 8;
       ...
       if s < start_pos - ego_para.rear2front || ...
          s > end_pos + ego_para.rear2back % if s < start_pos - ego_para.rear2front * 0.1 || s> end_pos + ego_para.rear2back
           obstacle_avoid_bound.lb_head(idx, 1) = ...
                   - road_width / 2 + obs_info.width / 2 * 0.0; % Todo : ignore constraints between front steer && rear steer
           obstacle_avoid_bound.ub_head(idx, 1) = ...
                   road_width / 2 - obs_info.width / 2 * 0.0;
           ...
           obstacle_avoid_bound.lb_head(idx, 1) = ...
                   - road_width / 2 + safe_distance + obs_info.width / 2 * 0.0;
           obstacle_avoid_bound.ub_head(idx, 1) = ...
                   - obs_info.width / 2 - safe_distance - obs_info.width / 2 * 0.0;
