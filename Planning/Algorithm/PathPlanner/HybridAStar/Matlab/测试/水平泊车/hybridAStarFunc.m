...
function [refPath, openList, closedList] = hybridAStarFunc(startPose, goalPose, scenario, paraCfg, gridMap, hCostMatrix)
    ...
    while ~isempty(openList) && searchTimesHA <= paraCfg.hAStar.maxSearchTimes % openList为空 || 搜索次数大于上限，认为Hybrid AStar失效
        ...
        [openList, minCostNode] = updateOpenList(openList, gridMap, paraCfg, closedList, hCostMatrix, goalPose);
...
%% 更新openList--剪枝
function [openList, minCostNode] = updateOpenList(openList, gridMap, paraCfg, closedList, hCostMatrix, goalPose)
    [minCostNode, openList] = removeNodeFunc(openList); % 移除openList中f最小的节点
    
    childNodes = searchChildNodesFunc(minCostNode, gridMap, paraCfg, hCostMatrix, goalPose); % 拓展节点
...
%% 拓展子节点
function childNodes = searchChildNodesFunc(minCostNode, gridMap, paraCfg, hCostMatrix, goalPose)
    childNodes = [];
    ...
    for direction = [1 -1] % 行驶方向
        for steer = -paraCfg.hAStar.maxSteer : paraCfg.hAStar.angleResolution : paraCfg.hAStar.maxSteer % 前轮转角
            ...
            if ~isempty(pose)
                cost = calTotalCost(pose, minCostNode, paraCfg, steer, direction, gridMap, hCostMatrix, goalPose);
...
function nodeCost = calTotalCost(pose, minCostNode, paraCfg, deltaFront, motionDirection, gridMap, hCostMatrix, goalPose)
...
function hCost = hCostFunc(startPose, goalPose, hCostMatrix, gridMap, paraCfg)
    hUnconstrained = hUnconstrainedFunc(startPose, hCostMatrix, gridMap); % 无约束启发代价
...
function paraCfg = optSearchStrategy(paraCfg, scenario, hUnconstrained)
    ...
    elseif  hUnconstrained >= paraCfg.hAStar.disBeginRSSearch
        ...
        if searchTimesRS <= 300
            ...
            paraCfg.hAStar.searchStep = scenario.costMap.CellSize * 1.0; % 与垂直泊车不同！！！
            paraCfg.hAStar.angleResolution = 15.0 / 57.3; % 与垂直泊车不同！！！
        else
            paraCfg.hAStar.vRef = 0.6;
            paraCfg.hAStar.searchStep = scenario.costMap.CellSize * 1.0;
            paraCfg.hAStar.angleResolution = 15.0 / 57.3;
        end
    end
...
function unrealToRealPath = unrealToRealPathFunc(goalPose, poseFrontParking, paraCfg)
    ...
    disFrontParking = -0.3;
    numStep = ceil(abs(disFrontParking) / paraCfg.hAStar.lineResolution) + 1;
    lineRes = paraCfg.disFrontParking / (numStep - 1);
...
%% 计算前置停车点--垂直泊车，虚拟停车点前置；水平泊车，虚拟停车点后置或不需要 % 与垂直泊车不同！！！
