...
function scenario = sceneCreatorFunc()
    ...
    vehicle.maxSteer   = 45.0 / 57.3; % 最大前轮转角（不能用满，给控制留余量）
    vehicle.minCircle  = vehicle.wheelBase/tan(vehicle.maxSteer); % 最小转弯半径
    
    scenario.vehicle = vehicle;
    
    % 车辆起始、停止位置--gridMap.res = 0.25
    startPose = [1.0, 6.0,  pi/2]; % 初始点位置--车辆后轴中心（全局规划）
%     goalPose  = [1.0, 6.5,  pi/2]; % 停车点位置--车辆后轴中心
    goalPose  = [4.0, 9.0,  0]; % 停车点位置--车辆后轴中心
