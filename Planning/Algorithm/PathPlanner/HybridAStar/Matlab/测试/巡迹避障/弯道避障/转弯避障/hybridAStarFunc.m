...
function nodeInfo = saveNodeInfoFunc(disSum, deltaFront, vel, pose, parentPose, cost, gridMap)
        ...
        mapIdx.xID   = ceil((pose(1) - gridMap.xMin) / 0.2);
        mapIdx.yID   = ceil((gridMap.yMax - pose(2)) / 0.2);       
        mapIdx.yawID = ceil((mod2pi(pose(3)) / 1.0001 - gridMap.yawMin) / (10.0/57.3));
        nodeInfo.mapIdx     = mapIdx;     % 节点MAP索引值
        
        mapIdx.xID   = ceil((parentPose(1) - gridMap.xMin) / 0.2); % 重复计算
        mapIdx.yID   = ceil((gridMap.yMax - parentPose(2)) / 0.2);
        mapIdx.yawID = ceil((mod2pi(parentPose(3)) / 1.0001 - gridMap.yawMin) / (10.0/57.3));
...
function paraCfg = optSearchStrategy(paraCfg, scenario, hUnconstrained)
    ...
    paraCfg.hAStar.searchStep = scenario.costMap.CellSize * 4.414;
    ...
    paraCfg.hAStar.yawPruneThreshold = 10.0 / 57.3;
