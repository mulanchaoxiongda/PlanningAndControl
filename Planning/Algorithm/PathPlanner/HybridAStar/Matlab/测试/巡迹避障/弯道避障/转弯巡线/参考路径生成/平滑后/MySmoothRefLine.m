function [ref_line] = MySmoothRefLine(routing_line)
    ...
    for s = 0 : interval_sampling : len_line + 0.001
    ...
    Weight1 = 1.0; % x向加速度项权重
    Weight2 = 1.0; % x向加加速度项权重
    Weight3 = 2000; % x向位置误差项权重
   ...
   amount_frag = len_fragment / interval_sampling; % 每个片段内采点个数
    ...
    smooth_line(:, 1) = ref_line.x;
    smooth_line(:, 2) = ref_line.y;
    smooth_line(:, 3) = ref_line.theta;
    smooth_line(:, 4) = ref_line.kappa;
    
    save ref_line.mat smooth_line;
