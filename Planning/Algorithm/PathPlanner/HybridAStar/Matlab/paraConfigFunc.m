...
function cfg = paraConfigFunc(scenario)
    % 混合A*参数
    hAStar.maxSearchTimes    = 1000;                                      % 最大搜索次数
    hAStar.vRef              = 0.6;                                       % 参考速度
    hAStar.lineResolution    = 0.01;                                      % 线运动分辨率
    hAStar.angleResolution   = 30.0 / 57.3;                               % 前轮转角分辨率
    ...
    hAStar.searchStep        = scenario.costMap.CellSize * 2.5;           % 搜索步长
    ...
    hAStar.disBeginRSSearch  = 7.0;                                       % 启用RS搜索的初始曼哈顿相对距离--建议取值7~9m
    ...
    collisionDet.lenExpansion = 0.15;                       % 车身膨胀长度
    collisionDet.widExpansion = 0.15;                       % 车身膨胀宽度
    ...
    % 虚拟停车点前置长度
    cfg.disFrontParking = 1.8; % 停车起始位置可行域最大化
