...
ref_path = ref_path.refPath(:, 1 : 3);

% N = 601; % 直角转弯轨迹，相邻两点间距1cm
% for i = 1 : 1 : 251
...
% 
% for i = 252 : 1 : 601
%     ref_path(i, 1) = (251 - 1) / 100;
%     ref_path(i, 2) = (i - 251) / 100;
%     ref_path(i, 3) = pi / 2;
% end

gate_noise = 0; % 噪声开关----routing_line点位置无噪声
for i = 1 : 1 : size(ref_path, 1)
    ref_path(i, 1) = ref_path(i, 1) + normrnd(0, 0.03) * gate_noise;
    ref_path(i, 2) = ref_path(i, 2) + normrnd(0, 0.03) * gate_noise;
    ref_path(i, 3) = ref_path(i, 3) + normrnd(0, 0.5 / 57.3) * gate_noise;
end

% plot(ref_path(:, 1), ref_path(:, 2), 'b'); axis equal; hold on;

interval = 10; % 路径点间距 interval cm
